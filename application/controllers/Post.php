...
class Post extends CI_Controller
{
    ...
    public function ajaxGetPostList() 
    {
    	...
    	$sql="SELECT tmt.*
    				...
    				WHERE (tmt.from_member_id='".$user_auto_id."' OR tmt.to_member_id='".$user_auto_id."' ) AND tmt.status='S' AND tmt.deleted='0' AND tp.deleted='0' AND tm.status='1' and tm.deleted='0' group by tmt.post_id order by tmt.create_date DESC  limit ".$row.",".$rowperpage;
		...
		$returnData['status']='1';
        $returnData['msg']='success';
        $returnData['msgstring']=$sql;
        $returnData['data']=array('postScrollData'=>$finalPost);
        echo json_encode($returnData);
        exit;
